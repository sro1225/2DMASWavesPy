...
class Dataset():
    ...
    def add_from_textfile(self, record_id, file_name, header_lines, n, direction, dx, x1, geometry_file=None, **kwargs):
        ...
        self.records[record_id] = RecordMC.import_from_textfile(self.site, self.profile, file_name, header_lines, n, direction, 
                                                                dx, x1, self.fs, self.f_pick_min, geometry_file=geometry_file, **kwargs)
    ...
    def records_from_csv(self, import_format, csv_file):
        ...
        no_records = len(df) 
        if "geometry_file" not in df.columns:
            df["geometry_file"] = None
        if import_format == 'textfile':
            for i in range(no_records):
                self.add_from_textfile(df['record_id'][i], df['file_name'][i], 
                                       df['header_lines'][i], df['n'][i],  
                                       df['direction'][i], df['dx'][i], df['x1'][i], geometry_file=df['geometry_file'][i])
        else:
            for i in range(no_records):
