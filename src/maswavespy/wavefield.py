...
class RecordMC():
    ...
    def __init__(self, site, profile, traces, n, direction, dx, x1, fs, f_pick_min, file_name=None, metadata=None, geometry_file=None):      
        ...
        self.sources = None
        self.receivers = None
        if geometry_file is not None and str(geometry_file) != "nan":
#            geom = pd.read_csv(os.path.join(os.path.dirname(file_name),geometry_file))
            geom = pd.read_csv(geometry_file)
            self.sources = geom["source"].to_numpy()
            self.receivers = geom["receiver"].to_numpy()
            self.offsets = geom["offset"].to_numpy()
    ...
    def _check_direction_str(direction):
        ...
        ------
        ValueError
            If 'direction' is not specified as 'forward', ´reverse´, 'midspread' or 'CMP'
       
        """
        directions = ['forward', 'reverse', 'midspread', 'cmp']
        if direction.lower() not in directions:
            message = f'direction must be specified as ´forward´, ´reverse´, ´midspread´, or ´cmp´ not as ´{direction}´'
    ...
    @classmethod
    def import_from_textfile(cls, site, profile, file_name, header_lines, direction, dx, x1, fs, f_pick_min, n=None, metadata=None, geometry_file=None, **kwargs):  
...
#        print(f"KWARGS:{kwargs}")
        ...
        return RecordMC(site, profile, traces, n, direction, dx, x1, fs, f_pick_min, file_name, metadata, geometry_file=geometry_file)  
    ...
    def plot_data(self, du=1.25, filled=True, normalized=True, translated=False, figwidth=9, figheight=12):
        ...
        # Forward shot
            ...
            x = self.x1 - np.arange(self.n) * self.dx
        elif self.direction == "CMP":
            plt.xticks(offsets[0::5],self.offsets[0::5])
#            plt.xticks(offsets[0::5], self.x1 + self.offsets * receiver[0::5])   
#            x = self.offsets
    ...
    def dispersion_imaging(self, cT_min, cT_max, cT_step):
        ...
        # Location vector used for phase-shift transform

        if self.direction == "CMP":
            x = self.offsets
        else:
            x = np.arange(
                self.x1,
                self.x1 + self.n * self.dx,
                self.dx
            )
